...
function generateDutyTable() {
  ...
  workers.forEach(worker => {
    ...
    label.textContent = `ימים שאינם מתאימים ל-${worker}`;
    form.appendChild(label);

    const daySelector = document.createElement('div');
    daySelector.className = 'day-select';
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      daySelector.appendChild(checkbox);
      daySelector.appendChild(cbLabel);
    }
    form.appendChild(daySelector);
  });
...
function buildCalendarWithAvailability() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    workerDiv.textContent = chosen.join(', ');
